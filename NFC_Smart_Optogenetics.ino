/*  Current state: V6 - Head mounted device with intensity control
 *   
 *   Supports:
...
volatile int  COUNT0 = 0;       // Gives the full period in ms for Channel 0
...
void setup() {
  ...
  DDR_Channels = 0x0F;              // Input RF WIP (PA7)
...
//    delay(50);
  ...
  while (FLAG0) {
    ...
    switch (MODE) {
      case 0:
        if (COUNT0 == 100) {
          bitClear(PORT_Indicator, Indicator);
        }
        if (COUNT0 == 1000)  {
          PORT_Indicator |= ((1 << Indicator) * (INDONOFF));
          COUNT0 = 0;
        }
        break;
      ...
      case 2:
        ...
        if (COUNT0 == CH2_DC1) {
          TIMSK1 = 0x00;  // Dissable interrupt timer 1
          bitClear(PORT_Channels, CH2);
          PORT_Indicator |= ((1 << Indicator) * (INDONOFF));
          ONOFF = 0x00;
        }
        if (COUNT0 == CH2_T1)  {
          TIMSK1  = (1 << OCIE1A) | (1 << OCIE1B); // Enables interruption A and B.
          bitSet(PORT_Channels, CH2);
          bitClear(PORT_Indicator, Indicator);
          TCNT1H = 0x00;  TCNT1L = 0x00;
          COUNT0 = 0;     ONOFF = 0xFF;
        }
        ...
        if (COUNT0 == CH3_DC1) {
          TIMSK1 = 0x00;  // Dissable interrupt timer 1
          bitClear(PORT_Channels, CH3);
          PORT_Indicator |= ((1 << Indicator) * (INDONOFF));
          ONOFF = 0x00;
        }
        if (COUNT0 == CH3_T1)  {
          TIMSK1  = (1 << OCIE1A) | (1 << OCIE1B); // Enables interruption A and B.
          bitSet(PORT_Channels, CH3);
          bitClear(PORT_Indicator, Indicator);
          TCNT1H = 0x00;  TCNT1L = 0x00;
          COUNT0 = 0;     ONOFF = 0xFF;
        }
      ...
      case 3: case 5: case 6: case 9: case 10: case 12:
        if (PHASE == 0) {   // Out of Phase
          ...
          if (COUNT0 == DC1_inPhase) {
            TIMSK1 = 0x00;  // Dissable interrupt timer 1
            PORT_Channels = 0xF0;
            PORT_Indicator |= ((1 << Indicator) * (INDONOFF));   // Peripheral - UCincinnati
            ONOFF = 0x00;
          }
